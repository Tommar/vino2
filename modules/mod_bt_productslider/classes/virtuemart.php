...
class BtProductSliderVirtuemartDataSource extends BTProductSliderSource {
    ...
    function buildQuery($category_ids = false, $product_ids = false) {
        ...
        $select[] = '`#__virtuemart_products`.`product_parent_id` as parent_id';
    ...
    function reloadProducts(&$products) {
        ...
        foreach ($tmp as $product) {
            if ($product->parent_id && (($this->_params->get('show_category_name') && !$product->category_name) || ($this->_params->get('show_manufacturer') && !$product->manufacturer_name))) {
                $parent = false;
                ...
                while ($product->parent_id) {
                    if (key_exists($product->parent_id, $tmp)) {
                        $parent = $tmp[$product->parent_id];
                    } else {
                        //thuc hien truy van
                        $db = JFactory::getDbo();
                        $query = $this->buildQuery(false, array($product->parent_id)) ;
                        $db->setQuery($query);
			...
			foreach ($tmp as &$product){	
				...
				if ($product->showRating) 
				{
					...
					if (empty($product->rating)) {
						...
						$product->ratingSpan.= '<span class="" style="width: ' . $ratingwidth.'%"></span>';
