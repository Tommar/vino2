...
defined('_JEXEC') or die;
	...
	$config=JURI::root().'templates/'.$template.'/html/mod_bt_smartsearch/images/config.jpg';
	$output = '<div class="borderinput"><input name="q" id="mod-bt-smartsearchword"  class="input inputboxsearch" type="text" value="'. ($value ? $value : $opensearch_title).'"  onblur="if (this.value==\'\') this.value=\''.$opensearch_title.'\';" onfocus="if (this.value==\''.$opensearch_title.'\') this.value=\'\';" />';
	if($show_advanced  == 1){
	$output .='<a id ="btss-advanced"><img src="'.$config.'"  alt="config"/></a>';	}
	$output .='</div>';	
	$image=JURI::root().'templates/'.$template.'/html/mod_bt_smartsearch/images/button.png';
	$button = '<button type="submit" class="btnsearch" >
					<img src="'.$image.'" width="17" height="17" alt="submit" />
			   </button>';	
	if($show_button ==1){
		switch($button_pos){
		case'top':			
			...
			break;
		case'left':
			$document->addStyleDeclaration('.borderinput{margin-left:63px;} .btnsearch{position:absolute;top: 4px; left: 10px;}');				
			break;
		case'right':
			$document->addStyleDeclaration('.borderinput{margin-right:0px;} .btnsearch{position:absolute;top: 4px; right: 10px;}');		
			break;
		case'bottom':
